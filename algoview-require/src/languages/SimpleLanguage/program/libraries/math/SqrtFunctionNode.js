...
VariableNameNode, VariableTypeNode, FloatDataType, FloatMemoryValue) {
	...
	function SqrtFunctionNode() {
		BuiltInFunctionNode.call(this);
		
		// Name
		this.children.push(new FunctionNameNode(undefined, undefined, "sqrt"));
	...
	SqrtFunctionNode.prototype = new BuiltInFunctionNode();
	SqrtFunctionNode.prototype.constructor = SqrtFunctionNode;

	SqrtFunctionNode.prototype.executeBuiltInFunction = function(nodeContext, memory, nodeStack, programRunner) {
		// TODO: Convert parameter in Float
		nodeContext.setValue(new FloatMemoryValue(Math.sqrt(nodeContext.parametersValues.children[0].value.value)));
	}

	return SqrtFunctionNode;
