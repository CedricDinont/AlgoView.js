...
VariableNameNode, VariableTypeNode, FloatDataType, FloatMemoryValue) {
	...
	function AbsFunctionNode() {
		BuiltInFunctionNode.call(this);
		
		// Name
		this.children.push(new FunctionNameNode(undefined, undefined, "abs"));
	...
	AbsFunctionNode.prototype = new BuiltInFunctionNode();
	AbsFunctionNode.prototype.constructor = AbsFunctionNode;

	AbsFunctionNode.prototype.executeBuiltInFunction = function(nodeContext, memory, nodeStack, programRunner) {
		// TODO: Convert parameter in Float
		nodeContext.setValue(new FloatMemoryValue(Math.abs(nodeContext.parametersValues.children[0].value.value)));
	}

	return AbsFunctionNode;
